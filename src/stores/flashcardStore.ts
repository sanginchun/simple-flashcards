...
interface FlashcardState {
  ...
  addCard: () => void;
  updateCard: (cardId: string, field: 'front' | 'back', value: string) => boolean;
...
export const useFlashcardStore = create<FlashcardState>()(
  immer((set, get) => ({
    ...
    deleteCard: (cardId: string) => {
      set((state) => {
        state.list.cards = state.list.cards.filter(card => card.id !== cardId);
        state.hasUnsavedChanges = true;
      });
    },
    ...
    isValidForSaving: () => {
      ...
      if (!list.title.trim()) {
        return { isValid: false, error: 'Please enter a title for your flashcard set!' };
      }
      ...
      const hasEmptyCards = list.cards.some(card => !card.front.trim() || !card.back.trim());
      if (hasEmptyCards) {
        return { isValid: false, error: 'Please fill in all flashcard fields!' };
      }
